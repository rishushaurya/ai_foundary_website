    ...
    <div className="mesh-bg min-h-screen pt-36 sm:pt-44 pb-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
        ...
        <div className="flex items-center justify-center gap-2">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="w-24 h-9 rounded-full bg-slate-200 animate-pulse" />
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div
              key={i}
              className="glass-card rounded-3xl h-72 bg-white/60 animate-pulse border border-white/80"
            />
          ))}
        </div>
      </div>
